...
const ChatApp = () => {
  ...
  const getdata = async ()=>{
    ...
    let config = {
      ...
      url: `http://localhost:5000/user/getdata/${sid}`,
      headers: {},
    };
    ...
    axios
      ...
      .then((response) => {
        ...
        setPincode(response.data.data[0].pincode);
        console.log(address);
        console.log(pincode);
      })
      .catch((error) => {
  ...
  return (
    <div>
      <div className="m-0 pb-3 my-20">
        <div>
          {id_c == tid && (
            <div className="absolute top-15 mx-auto p-5 bg-[#c6c5c5] right-4">
              <div>Address of the user : {address}</div>
          ...
          <div>
            {id_c == tid && (
              <button
                ...
                className="bg-[#333] py-[1rem] m-[0.25rem] rounded-md px-6  text-white"
              >
                Finish
              </button>
            )}
